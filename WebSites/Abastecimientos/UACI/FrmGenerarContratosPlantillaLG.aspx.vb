...
Partial Class frmGenerarContratosPlantillaLG
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Me.ddlTIPODOCUMENTOCONTRATO1.Items.FindByText("Orden de compra").Selected = True
            Me.ddlTIPODOCUMENTOCONTRATO1.Visible = False
        ...
        Else
            ...
            If MessageBox.ConfirmTarget = "Guardado" Then IrAClausula()
            If MessageBox.ConfirmTarget = "LIBERAR" Then Liberar()
        End If
    ...
    Private Sub cargarProveedores()
        Me.dgProveedor.DataSource = cPC.obtenerProveedoresContratoDS_LG(Session("IdEstablecimiento"), Request.QueryString("IdProc"), Session("IdEmpleado"))
    ...
    Private Sub obtenerPlantillasContrato(ByVal IDTIPOCOMPRA As Integer, ByVal IDSUMINISTRO As Integer)
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            'End If
        Else
            MessageBox.Alert("No hay plantillas definidas, consulte al administrador del sistema", "PLANTILLAS")
    ...
    Protected Sub dgProveedor_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgProveedor.SelectedIndexChanged
        ...
        If Me.IDESTADOCONTRATO = 2 Then
            ...
            contrato = "OC" & IDCONTRATO & "PROV" & IDPROVEEDOR & ".doc"
    ...
    Protected Sub LinkButton2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkButton2.Click
        Response.Redirect("~/UACI/frmGenerarContratosLG.aspx")
    End Sub
    ...
    '    Dim eC As CONTRATOS
    '    eC = mComponente.ObtenerCONTRATOS(Session("IdEstablecimiento"), IDPROVEEDOR, IDCONTRATO)
    ...
    Private Function guardarContrato() As Integer
        ...
        If validaExisNumContrato = True Then
            ...
            Return 1
        Else
            MessageBox.Alert("El número de orden de compra ya existe, debe ingresar otro número", "Repetido")
            'Me.MsgBox2.ShowAlert("El número de orden de compra ya existe, debe ingresar otro número", "", Cooperator.Framework.Web.Controls.AlertOption.NoAction, Cooperator.Framework.Web.Controls.AlertType.Exclamation)
            Return 0
        End If
    ...
    '    Dim GARANTIACUMPLIMIENTOORDENCOMPRA As Decimal

    '    With ds.Tables(0).Rows(0)
    '        GARANTIAMTTOENTREGA = 0
    '        GARANTIAMTTOVIGENCIA = 0
    '        GARANTIACUMPLIMIENTOVALOR = 0
    '        GARANTIACUMPLIMIENTOENTREGA = 0

    '        GARANTIACUMPLIMIENTOVIGENCIA = 0
    '        If IsDBNull(.Item("GARANTIACALIDADVALOR")) Then
    '            GARANTIACALIDADVALOR = 0
    '        Else
    '            GARANTIACALIDADVALOR = .Item("GARANTIACALIDADVALOR")
    '        End If
    '        If IsDBNull(.Item("GARANTIACUMPLIMIENTOORDENCOMPRA")) Then
    '            GARANTIACUMPLIMIENTOORDENCOMPRA = 0
    '        Else
    '            GARANTIACUMPLIMIENTOORDENCOMPRA = .Item("GARANTIACUMPLIMIENTOORDENCOMPRA")
    '        End If

    '        GARANTIACALIDADENTREGA = 0
    '        GARANTIACALIDADVIGENCIA = 0
    '    End With
    ...
    '        .AUUSUARIOCREACION = HttpContext.Current.User.Identity.Name
    '        .ESTASINCRONIZADA = 0
    '        .FECHAENTREGA = mComContrato.ObtenerDataSetPorId(Session("IdEstablecimiento"), IDPROVEEDOR, IDCONTRATO).Tables(0).Rows(0).Item("FECHAINICIOENTREGA")
    ...
    Protected Sub ImageButton1_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton1.Click
        ...
        If validaOrden() = False Then
            ...
            If mComponente.ActualizarCLAUSULASCONTRATOS(lEntidad) <> 1 Then
                MessageBox.Alert("El registro no ha sido almacenado, verifique sus datos o informe al administrador", "Aviso")
            ...
            Else
                MessageBox.AlertSubmit("El registro se ha almacenado satisfactoriamente, a continuación regresará al listado de clausulas.", "Guardado")
        ...
        Else
            MessageBox.Alert("El número de orden ya existe favor verificar.", "Repetido")
    ...
    Protected Sub MsgBox1_OkChosen(ByVal sender As Object, ByVal e As Cooperator.Framework.Web.Controls.MsgBoxEventArgs) Handles MsgBox1.OkChosen
        ...
        ElseIf e.Key = "LIBERAR" Then
            ...
            Me.lblCargaArchivo.Text = "La orden de compra fue liberado satisfactoriamente, desde este momento solo podrá realizar consultas a la orden de compra generada."
            Label17.Text = "La orden de compra ya fue liberado para este proveedor, puede descargarlo en el siguiente opción"
        End If
    ...
    Private Function ElaborarContrato() As StringBuilder
        ...
        Encabezado.Append("<body><table cellspacing=1 style='font-family:Arial;' cellpadding=2 width=100% border=0><tbody><tr><td></td><td style='text-align: right;'></td></tr></TBODY></table>")
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            With ds.Tables(0)
                'PERSONERIAJURIDICA
                textoContrato.Append("<table style='font-family:Arial; text-align:justify; width:100%'><tr><td>")
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    textoContrato.Append(.Rows(i).Item("CONTENIDO"))
                Next
    ...
    Private Sub reemplazarEtiquetas(ByVal textoContrato As StringBuilder)
        ...
        textoReemplazado = Replace(textoReemplazado.ToString, "$NUMERO_CONTRATO$", Server.HtmlEncode(Me.txtNumContrato.Text))

        'obteniendo lugar y direccion de entrega y dependencia solicitante
        Dim cspc As New cSOLICITUDESPROCESOCOMPRAS
        Dim dsspc As Data.DataSet
        dsspc = cspc.ObtenerSolicitudesProcesoCompra1(Request.QueryString("idProc"), Session("IdEstablecimiento"))
        textoReemplazado = Replace(textoReemplazado.ToString, "$DEPENDENCIA_SOLICITANTE$", Server.HtmlEncode(dsspc.Tables(0).Rows(0).Item("DEPEDENCIASOLICITANTE")))

        If IsDBNull(dsspc.Tables(0).Rows(0).Item("IDESTABLECIMIENTOENTREGA")) Then
            textoReemplazado = Replace(textoReemplazado.ToString, "$LUGAR_ENTREGA$", Server.HtmlEncode(dsspc.Tables(0).Rows(0).Item("ALMACENENTREGA")))
            textoReemplazado = Replace(textoReemplazado.ToString, "$DIRECCION_LUGAR_ENTREGA$", Server.HtmlEncode(dsspc.Tables(0).Rows(0).Item("DIRECCIONALMACEN")))
        Else
            textoReemplazado = Replace(textoReemplazado.ToString, "$LUGAR_ENTREGA$", Server.HtmlEncode(dsspc.Tables(0).Rows(0).Item("ESTABLECIMIENTOENTREGA")))
            textoReemplazado = Replace(textoReemplazado.ToString, "$DIRECCION_LUGAR_ENTREGA$", Server.HtmlEncode(dsspc.Tables(0).Rows(0).Item("DIRECCIONESTABLECIMIENTO")))
        End If
        ...
        '    CIFRADOPRESUPUESTARIO = mComPC.obtenerCifradoPresupuestario(Session("IdEstablecimiento"), Request.QueryString("IdProc"))
        ...
        textoReemplazado = Replace(textoReemplazado.ToString, "$CONTRATISTA$", Server.HtmlEncode(lEntProveedor.NOMBRE))
        ...
        For i = 0 To dsRenglonesAdjudicados.Tables(0).Rows.Count - 1
            ...
            tablaProductos.Append("<td style='text-align: right'>" & FormatCurrency(CDec(dsRenglonesAdjudicados.Tables(0).Rows(i).Item("PRECIOUNITARIO"))) & "</td>")
            tablaProductos.Append("<td style='text-align: right'>" & FormatCurrency(CDec(dsRenglonesAdjudicados.Tables(0).Rows(i).Item("VALORTOTAL"))) & "</td></tr>")
        ...
        'Dim cpc2 As New cPROCESOCOMPRAS
        'Dim ds2 As Data.DataSet
        'ds2 = cpc2.obtenerTipoProcesoCompra(Session("IdEstablecimiento"), Request.QueryString("idProc"))
        ...
        textoReemplazado = Replace(textoReemplazado, "$GARANTIA_CUMPLIMIENTO_ORDENCOMPRA$", Server.HtmlEncode(ds.Tables(0).Rows(0).Item("GARANTIACUMPLIMIENTOORDENCOMPRA")))
        textoReemplazado = Replace(textoReemplazado, "$GARANTIA_CALIDAD_VALOR$", Server.HtmlEncode(ds.Tables(0).Rows(0).Item("GARANTIACALIDADVALOR")))
        ...
        contrato = "OC" & IDCONTRATO & "PROV" & IDPROVEEDOR & ".htm"

        directorio = "EST" & Session("IdEstablecimiento") & "_PROC" & IDPROCESOCOMPRA

        If Directory.Exists(Server.MapPath(directorio)) = False Then
            MessageBox.Alert("Verifique la configuración en el servidor, la carpeta " & directorio & " no ha sido creada", "Error")
        ...
        Else
            If Directory.Exists(Server.MapPath(directorio & "\CONTRATOS\")) = False Then
                MessageBox.Alert("Verifique la configuración en el servidor, la carpeta CONTRATOS de la carpeta " & directorio & " no ha sido creada", "Error")
            ...
            Else
                ...
                Me.fuContrato.Visible = True
                MessageBox.Alert("La orden de compra se generó satisfactoriamente", "Orden Generada")
                'Me.MsgBox2.ShowAlert("La orden de compra se generó satisfactoriamente", "", Cooperator.Framework.Web.Controls.AlertOption.NoAction, Cooperator.Framework.Web.Controls.AlertType.Information)
    ...
    Protected Sub btnCargarArchivo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCargarArchivo.Click
        ...
        If fuContrato.HasFile Then
            Try
                ...
                btnLiberaContrato.Visible = True
                MessageBox.Alert("El archivo se cargó satisfactoriamente", "Cargado")
    ...
    Protected Sub btnLiberaContrato_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLiberaContrato.Click
        MessageBox.Confirm("¿Desea liberar el contrato?, una vez liberado no podrá realizar modificaciones.", "LIBERAR", MessageBox.OptionPostBack.YesPostBack)
    ...
    Protected Sub Liberar() ' MsgBox2_YesChosen1(ByVal sender As Object, ByVal e As Cooperator.Framework.Web.Controls.MsgBoxEventArgs) Handles MsgBox2.YesChosen
        'Select Case e.Key
        '  Case Is = "LIBERAR"
        Dim lEntidad As New CONTRATOS
        ...
        Me.btnCargarArchivo.Visible = False
        btnGenerarContrato.Visible = False
        Me.lblCargaArchivo.Text = "La orden de compra fue liberado satisfactoriamente, desde este momento solo podrá realizar consultas al contrato generado."
