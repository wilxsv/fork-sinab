...
Partial Class FrmConsultaSolicitudCompraLG
    ...
    Private Sub CargarDatos()
        ...
        ds = Me.mComponente.ObtenerSolicitudesLG(User.Identity.Name)
    ...
    Protected Sub gvLista_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles gvLista.RowDeleting
        ...
        Dim IDESTABLECIMIENTOSOLICITUD As Integer = Me.gvLista.DataKeys(e.RowIndex).Values.Item("IDESTABLECIMIENTO")

        Try
            'Dim mCompDetalleEntregas As New cDETALLEENTREGAS
            'mCompDetalleEntregas.EliminarDETALLEENTREGASxSolicitud(IDSOLICITUD, IDESTABLECIMIENTOSOLICITUD)

            'elimina entregas
            Dim mCompEntregas As New cENTREGASOLICITUDES
            mCompEntregas.EliminarENTREGASXSolicitud(IDSOLICITUD, IDESTABLECIMIENTOSOLICITUD)

            'elimina fuente de financiamiento
            Dim mCompFuenteFinancSol As New cFUENTEFINANCIAMIENTOSOLICITUDES
            mCompFuenteFinancSol.EliminarFUENTESSOLICITUD(IDSOLICITUD, IDESTABLECIMIENTOSOLICITUD)

            'elimina detalle de solicitudes
            Dim mCompDetalleSolicitud As New cDETALLESOLICITUDES
            mCompDetalleSolicitud.EliminarDetallesSolicitud(IDSOLICITUD, IDESTABLECIMIENTOSOLICITUD)

            'elimina de necesidades solicitud para compra conjunta
            'Dim mCompNecesidadesSolicitud As New cNECESIDADESSOLICITUD
            'mCompNecesidadesSolicitud.EliminarSolicitud(IDSOLICITUD, IDESTABLECIMIENTOSOLICITUD)

            'elimina solicitud
            Me.mComponente.EliminarSOLICITUDES(IDESTABLECIMIENTOSOLICITUD, IDSOLICITUD)

            Me.CargarDatos()
            MsgBox1.ShowAlert("Su solicitud fue eliminada", "A", Cooperator.Framework.Web.Controls.AlertOption.NoAction)
        Catch ex As Exception
            Response.Write("<script language='JavaScript'>alert('No se puede eliminar la solicitud porque ya ha iniciado un proceso de compra.')</script>")
        End Try
    ...
    Protected Sub BttBuscar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BttFiltrar.Click
        ...
        If Me.DdlCriterio.SelectedItem.Value = "CORRELATIVO" Then
            ...
            Me.gvLista.DataSource = mComponente.Filtrar(textoBusqueda, criterioBusqueda, operadorBusqueda, User.Identity.Name)
        ...
        If Me.DdlCriterio.SelectedItem.Value = "FECHASOLICITUD" Then
            ...
            criterioBusqueda = Me.DdlCriterio.SelectedItem.Value
            Me.gvLista.DataSource = mComponente.FiltrarRangosFecha(FechaInicio, FechaFin, criterioBusqueda, User.Identity.Name)
            Try
                Me.gvLista.DataBind()
            Catch ex As Exception
                Me.gvLista.PageIndex = 0
