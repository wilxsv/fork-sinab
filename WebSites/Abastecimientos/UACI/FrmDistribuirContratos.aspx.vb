...
Partial Class FrmDistribuirContratos
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        If Me.gvOfertas.SelectedIndex = -1 Then
            Alert("Debe seleccionar una oferta para poder imprimir el contrato.", "Alerta")
        ...
        Else
            Dim path As String
            path = Server.MapPath("EST" & Session("IdEstablecimiento") & "_PROC" & Request.QueryString("idProc") & "\CONTRATOS\C" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(1).ToString & "PROV" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(0).ToString & ".doc")

            If File.Exists(path) Then
                Dim c As New CONTRATOS
                cC.ActualizarFechaDistribucion(CType(cpFechaDistribucion.Text, Date), Request.QueryString("idProc"), Session("IdEstablecimiento"), Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(1).ToString, Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(0).ToString)
            ...
            Else
                Alert("El archivo C" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(1).ToString & "PROV" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(0).ToString & ".doc no se encuentra. Favor revisar si el contrato ya ha sido generado.", "ERROR")
                'Me.MsgBox1.ShowAlert("El archivo C" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(1).ToString & "PROV" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(0).ToString & ".doc no se encuentra. Favor revisar si el contrato ya ha sido generado.", "M", Cooperator.Framework.Web.Controls.AlertOption.PostBackOnOk)
            End If
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        If QP = QPC Then
            If cC.ChequearContratosDistribuidos(Session("IdEstablecimiento"), Request.QueryString("idProc")) Then
                ...
                AlertSubmit("El proceso de compra ha cambiado de estado.", "Cambio")
            ...
            Else
                Alert("Existen contratos que no han sido distribuidos aún. El proceso de compra no podrá cambiar de estado.", "Omitido")
                'Me.MsgBox2.ShowAlert("Existen contratos que no han sido distribuidos aún. El proceso de compra no podrá cambiar de estado.", "O", Cooperator.Framework.Web.Controls.AlertOption.PostBackOnOk)
            End If
        Else
            Alert("Existen contratos que no han sido distribuidos aún. El proceso de compra no podrá cambiar de estado.", "Omitido")
            'Me.MsgBox2.ShowAlert("Existen contratos que no han sido distribuidos aún. El proceso de compra no podrá cambiar de estado.", "O", Cooperator.Framework.Web.Controls.AlertOption.PostBackOnOk)
    ...
    Protected Sub gvOfertas_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvOfertas.SelectedIndexChanged
        ...
        Dim Contrato As String = "C" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(1).ToString & "PROV" & Me.gvOfertas.DataKeys(Me.gvOfertas.SelectedIndex).Values(0).ToString
