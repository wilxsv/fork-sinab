...
Partial Class FrmDetMantDespachoDependencias
    ...
    Private Function CargarDatos() As Int16
        If Operacion = "Edicion" Then
            ...
            mEntVale.IDVALE = mEntMovimientos.IDDOCUMENTO
            mCompVale.ObtenerVALESSALIDA(mEntVale)
            Me.DdlTIPOIDENTIFICACION1.SelectedValue = mEntVale.IDTIPOIDENTIFICACION
            Me.txtPersonaRecibe.Text = mEntVale.PERSONARECIBE
            Me.TxtNumeroDocumento.Text = mEntVale.NUMEROIDENTIFICACION
    ...
    Protected Sub MsgBox1_YesChosen(ByVal sender As Object, ByVal e As Cooperator.Framework.Web.Controls.MsgBoxEventArgs) Handles MsgBox1.YesChosen
        If e.Key = "Q1" Then 'Verifica si el usuario desea crear el documento
            ...
            mEntMovimientos.IDESTABLECIMIENTODESTINO = Session.Item("IdEstableciento")
            ...
            mEntVale.ANIO = mEntMovimientos.ANIO
            mEntVale.IDVALE = 0
            mEntVale.IDTIPOIDENTIFICACION = Me.DdlTIPOIDENTIFICACION1.SelectedValue
            mEntVale.PERSONARECIBE = Me.txtPersonaRecibe.Text
            mEntVale.NUMEROIDENTIFICACION = Me.TxtNumeroDocumento.Text
            mEntVale.OBSERVACION = Me.TxtObservacion.Text
        ...
        If e.Key = "Q2" Then 'Verifica si el usuario desea cerrar el documento
            ...
            mCompMovimientos.ObtenerMOVIMIENTOS(mEntMovimientos)
            Me.DOCUMENTOENLACE = mEntMovimientos.NUMERODOCREF
            mEntMovimientos.IDESTADO = 2 'Cambio del estado del documento de grabado a cerrado
            If mEntMovimientos.AUUSUARIOCREACION = Nothing Then
                mEntMovimientos.AUUSUARIOCREACION = HttpContext.Current.User.Identity.Name
            End If
            If mEntMovimientos.AUFECHACREACION = Nothing Then
    ...
    Protected Sub ImgbImprimir_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgbImprimir.Click
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "VistaPrevia", "<script language='jscript'> window.open('" + Request.ApplicationPath + "/Reportes/FrmRptValeSalidaDependencia.aspx?IdMovimiento=" + Me.TxtIddocumento.Text + "&IdAlmacen=" + Me.TxtIdAlmacen.Text + "&IdAnio=" + Str(Me.CalendarFechaDespacho.SelectedDate.Year) + "', 'popup' ,'scrollbars= 1 ,resizable= 1 ,width= 800 ,height= 600 '); </script>")
    End Sub
    ...
    Protected Sub gvProductosRequisicion_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvProductosRequisicion.RowDataBound
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                mEntExistenciasAlmacenes.IDPRODUCTO = Val(e.Row.Cells(1).Text)
                ...
                If mEntExistenciasAlmacenes.CANTIDADDISPONIBLE = 0 Then
                    e.Row.Cells(5).Controls(3).Visible = True
    ...
    Protected Sub gvProductosRequisicion_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvProductosRequisicion.SelectedIndexChanged
        ...
        Me.DdlLOTES2.RecuperarListaPorProductoFiltrado(Session.Item("IdAlmacen"), Me.gvProductosRequisicion.SelectedRow.Cells(1).Text, 1)
        dsLoteBusqueda = mCompLote.SeleccionarLoteDs(Session.Item("IdAlmacen"), Me.gvProductosRequisicion.SelectedRow.Cells(1).Text, 1)
        ...
        Else
            ...
            Me.TxtCantidadDistribucion.Text = Me.gvProductosRequisicion.SelectedRow.Cells(4).Text
    ...
    Protected Sub BtnGuardarDistribucion_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGuardarDistribucion.Click
        ...
        If Val(Me.TxtCantidadDistribucion.Text) > Val(Me.gvProductosRequisicion.SelectedRow.Cells(4).Text) Then
        ...
        mEntDetalleMovimiento.IDPRODUCTO = Me.gvProductosRequisicion.SelectedRow.Cells(1).Text
    ...
    Protected Sub DdlTIPOIDENTIFICACION1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles DdlTIPOIDENTIFICACION1.Load

    End Sub

    Protected Sub DdlTIPOIDENTIFICACION1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DdlTIPOIDENTIFICACION1.SelectedIndexChanged
