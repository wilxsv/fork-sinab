...
Partial Class ESTABLECIMIENTOS_frmMantConsumo2
    ...
    Private Function Actualizar() As String
        ...
        For Each row As GridViewRow In gvLista.Rows
            ...
            eComponente.IDESTABLECIMIENTO = CType(Session.Item("IdEstablecimiento"), Integer)
    ...
    Private Function actualizar2() As String
        ...
        ' validar que no existan ingresos de consumos cuando la existencia es cero y cuando el consumo es mayor a la existencia. (FARMACIA)
        If Me.ViewState("FARMACIA") = 1 Then
            Dim txtConsumo As eWorld.UI.NumericBox
            For Each row As GridViewRow In gvLista.Rows
                txtConsumo = row.FindControl("txtConsumo")

                If Me.gvLista.DataKeys(row.RowIndex).Values(2) = "CTO" Then
                    'If (txtConsumo.Text <> "0.00" And Me.gvLista.DataKeys(row.RowIndex).Values(4) = "0") Then
                    '    Alert("Existen productos que se les ha ingresado consumos, pero tienen existencia cero. El cálculo de la existencia no puede ser negativo. Deberá ajustar la existencia para que el cálculo se pueda realizar.", "Advertencia")
                    '    Exit Function
                    'End If
                    'If CDec(txtConsumo.Text) > CDec(Me.gvLista.DataKeys(row.RowIndex).Values(4)) Then
                    '    Alert("Existen productos que se les ha ingresado un consumo mayor a la existencia, lo cual no es permitido ya que la existencia no podrá ser negativa. Favor corregir el consumo o realizar un ajuste a la existencia.", "Advertencia")
                    '    Exit Function
                    'End If

                    If CDec(txtConsumo.Text) <> CType(row.Cells(5).FindControl("label2"), Label).Text Then
                        If CDec(txtConsumo.Text) > CDec(Me.gvLista.DataKeys(row.RowIndex).Values(4)) Then
                            Alert("Existen productos que se les ha ingresado un consumo mayor a la existencia, lo cual no es permitido ya que la existencia no podrá ser negativa. Favor corregir el consumo o realizar un ajuste a la existencia.", "Advertencia")
                            Exit Function
                        End If
                    End If
                Else
                    'If (txtConsumo.Text <> "0" And Me.gvLista.DataKeys(row.RowIndex).Values(4) = "0") Then
                    '    Alert("Existen productos que se les ha ingresado consumos, pero tienen existencia cero. El cálculo de la existencia no puede ser negativo. Deberá ajustar la existencia para que el cálculo se pueda realizar.", "Advertencia")
                    '    Exit Function
                    'End If
                    'If CInt(txtConsumo.Text) > CInt(Me.gvLista.DataKeys(row.RowIndex).Values(4)) Then
                    '    Alert("Existen productos que se les ha ingresado un consumo mayor a la existencia, lo cual no es permitido ya que la existencia no podrá ser negativa. Favor corregir el consumo o realizar un ajuste a la existencia.", "Advertencia")
                    '    Exit Function
                    'End If
                    If CInt(txtConsumo.Text) <> CInt(CType(row.Cells(5).FindControl("label2"), Label).Text) Then
                        If CInt(txtConsumo.Text) > CInt(Me.gvLista.DataKeys(row.RowIndex).Values(4)) Then
                            Alert("Existen productos que se les ha ingresado un consumo mayor a la existencia, lo cual no es permitido ya que la existencia no podrá ser negativa. Favor corregir el consumo o realizar un ajuste a la existencia.", "Advertencia")
                            Exit Function
                        End If
        ...
        For Each row As GridViewRow In gvLista.Rows
            ...
            If Me.ViewState("FARMACIA") = 1 And Not (txtAd.Text = "") Then
                ...
                Else
                    ...
                    Dim idalmacenhospital As Integer
                    idalmacenhospital = cCONs.obtenerIdAlmacenHospital(Session("IdEmpleado"))
                    ...
                    If Me.gvLista.DataKeys(row.RowIndex).Values(2) = "CTO" Then
                        ...
                        despachos = cCONs.obtenerDespachosDelMes(eComponente.IDESTABLECIMIENTO, idalmacenhospital, eComponente.FECHACONSUMO, eComponente.IDPRODUCTO)
    ...
    Private Sub CargarDatos() 'carga los datos y los muestra en el gridview
        ...
        Else
            If Me.ViewState("FARMACIA") = 0 Then
                'almacen
                ds = mComponente.obtenerExistenciaEstablecimientoFecha2(Session.Item("IdEstablecimiento"), Me.cboAlmacen.SelectedItem.Value, fecha, IIf(Me.cboAlmacen.SelectedItem.Text.ToUpper.Contains("FARMACIA"), 0, 1), 0)
            Else
                'farmacia
                ...
                Else
                    ...
                    Dim almacen = usr.Almacenes.FirstOrDefault(Function(al) al.ESFARMACIA = 1)
                    Dim idetablecimiento = usr.Establecimiento.IDESTABLECIMIENTO

                    ds = mComponente.obtenerExistenciaEstablecimientoFecha2(idetablecimiento, almacen.IDALMACEN, fecha, IIf(Me.cboAlmacen.SelectedItem.Text.ToUpper.Contains("FARMACIA"), 0, 1), usr.IDEMPLEADO) ' IIf(Me.cboAlmacen.SelectedItem.Text.ToUpper.Contains("FARMACIA"), 0, 1))
                End If
        ...
        Else
            'es farmacia
            If mComponente.ChequearCYEMEsPosterior(Session.Item("IdEstablecimiento"), Me.cboAlmacen.SelectedItem.Value, fecha, IIf(Me.cboAlmacen.SelectedItem.Text.ToUpper.Contains("FARMACIA"), 0, 1)) > 0 Then
                'ya hay datos del mes posterior, por lo que este mes solo es de solo lectura
                Alert("El mes consultado esta cerrado, y esta disponible solo para consultas.", "Advertencia")
    ...
    Protected Sub gvLista_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvLista.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim lb As Label
            lb = e.Row.FindControl("label2")
            lb.Text = IIf(IsDBNull(Me.gvLista.DataKeys(e.Row.RowIndex).Values(3)), 0, Me.gvLista.DataKeys(e.Row.RowIndex).Values(3))
            ...
            'Dim objAddress As CompareValidator = TryCast(e.Row.FindControl("CompareValidator1"), CompareValidator)
            'objAddress.ValidationGroup = c
            ''Dim objAdd As Button = TryCast(e.Row.FindControl("btnAdd"), Button)
            ''objAdd.ValidationGroup = groupName.ToString()
            'txtAd.ValidationGroup = c
            c += 1
            'txtAd.ValidationGroup = c
            ...
            existencia = e.Row.FindControl("txtExistencia")
            'txtAd.DecimalPlaces = IIf(Me.gvLista.DataKeys(e.Row.RowIndex).Values(2) = "CTO", 2, 0)
            'txtAd.RealNumber = IIf(Me.gvLista.DataKeys(e.Row.RowIndex).Values(2) = "CTO", True, False)
            ...
            If Me.cboAlmacen.SelectedItem.Text.ToUpper.Contains("FARMACIA") Then
                existencia.Enabled = True
            Else
                existencia.Enabled = False
            End If

            'Dim x As New TextBox
            'x = e.Row.FindControl("TextBox3")
            'x.Text = txtAd.Text

            txtAd.Attributes("data-valMax") = existencia.Text.ToString '"numero.toString xfa"
            txtAd.Attributes("onchange") = "validaTxt(this);"
            'Dim cv As New CompareValidator
            'cv.ID = "Cv" & c
            'cv.ControlToValidate = "txtConsumo"
            'cv.ErrorMessage = "Invalid Gender"
            'cv.SetFocusOnError = True
            'cv.Operator = ValidationCompareOperator.LessThanEqual
            'cv.ValueToCompare = existencia.Text
            'cv.Display = ValidatorDisplay.Dynamic
            'cv.ValidationGroup = c
            'cv.SetFocusOnError = True
        ...
        End If
    End Sub
    'Protected Sub txtConsumo_TextChanged(sender As Object, e As EventArgs)
    '    Dim row As GridViewRow = DirectCast(DirectCast(sender, TextBox).NamingContainer, GridViewRow)
    '    Dim txtConsumo As TextBox = DirectCast(row.FindControl("TxtConsumo"), TextBox)
    '    Dim lbconsumo As Label = DirectCast(row.FindControl("Label85"), Label)
    '    Dim cv1 As CompareValidator = DirectCast(row.FindControl("CompareValidator1"), CompareValidator)
    '    Dim exis As Decimal = row.Cells(10).Text

    '    If txtConsumo.Text <> lbconsumo.Text Then
    '        If txtConsumo.Text > exis Then
    '            cv1.Enabled = True
    '            cv1.Visible = True
    '            cv1.ValueToCompare = exis
    ...
    '            'cv1.ValueToCompare = exis
    '            'cv1.Operator = ValidationCompareOperator.GreaterThan
    '            'cv1.ControlToValidate = "TxtConsumo"
    '        End If
    '    End If
    ...
    Protected Sub btnCobertura_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCobertura.Click
        ...
        If Me.DropDownList1.SelectedIndex = 0 Then
            alerta = CType(("/Reportes/frmRptConsumo.aspx?mes=" & Me.cboMes.SelectedIndex & "&anio=" & Me.cboAnio.SelectedItem.Text & "&tipo=" & Me.ViewState("FARMACIA") & "&est=" & Me.cboAlmacen.SelectedItem.Value & "&c=1&idprog=0"), String)
        Else
            alerta = CType(("/Reportes/frmRptConsumo.aspx?mes=" & Me.cboMes.SelectedIndex & "&anio=" & Me.cboAnio.SelectedItem.Text & "&tipo=" & Me.ViewState("FARMACIA") & "&est=" & Me.cboAlmacen.SelectedItem.Value & "&c=1&idprog=" & Me.DropDownList1.SelectedValue.ToString), String)
        End If
    ...
    Protected Sub ImageButton2_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs)
        ...
        Me.lblUM2.Text = Me.gvLista.DataKeys(row.RowIndex).Values(2)
        Me.Label7.Text = Me.gvLista.DataKeys(row.RowIndex).Values(6)
    ...
    Protected Sub Button21_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Me.NumericBox11.Text.Trim = "" Then
            Me.Label81.Text = "Debe ingresar un valor al ajuste de existencia"
            Exit Sub
        End If

        If CDec(Me.lblConsumo2.Text) + CDec(Me.NumericBox11.Text) < 0 Then
            Me.Label81.Text = "La existencia no puede ser negativa posterior a un ajuste."
            Exit Sub
        End If

        If Me.NumericBox11.Text = 0 Then
            Me.Label81.Text = "El ajuste de existencia no puede ser igual a cero"
            Exit Sub
        End If

        If Me.DropDownList2.SelectedIndex = -1 Then
            Me.Label81.Text = "Debe seleccionar un motivo del ajuste"
            Exit Sub
        End If

        If Me.DropDownList2.SelectedIndex = 0 And Me.NumericBox11.Text > 0 Then
            Me.Label81.Text = "El motivo del ajuste seleccionado solo puede reducir la existencia. Anteponer signo negativo (-)"
            Exit Sub
        End If
        If Me.DropDownList2.SelectedIndex = 1 And Me.NumericBox11.Text > 0 Then
            Me.Label81.Text = "El motivo del ajuste seleccionado solo puede reducir la existencia. Anteponer signo negativo (-)"
            Exit Sub
        End If
        If Me.DropDownList2.SelectedIndex = 2 And Me.NumericBox11.Text > 0 Then
            Me.Label81.Text = "El motivo del ajuste seleccionado solo puede reducir la existencia. Anteponer signo negativo (-)"
            Exit Sub
        End If
        If Me.DropDownList2.SelectedIndex = 3 And Me.NumericBox11.Text < 0 Then
            Me.Label81.Text = "El motivo del ajuste seleccionado solo puede incrementar la existencia. El ajuste deberá ser positivo"
            Exit Sub
        End If
        ...
        If mdb.ajustarExistencia(eEntidad) = -1 Then
            ...
            Exit Sub
        Else
            mdb.IngresarMotivoExistenciaAjustada(Session.Item("IdEstablecimiento"), Me.cboAlmacen.SelectedValue, Me.lblIdProducto2.Text, fecha1, ExistenciaAjustada, Me.DropDownList2.SelectedValue.ToString())
    ...
    Protected Sub Button32_Click(sender As Object, e As System.EventArgs) Handles Button32.Click
        ...
        Else
            alerta = CType(("/Reportes/FrmRptConsumoAjusteExistencia.aspx?mes=" & Me.cboMes.SelectedIndex & "&anio=" & Me.cboAnio.SelectedItem.Text & "&tipo=" & Me.ViewState("FARMACIA") & "&est=" & Me.cboAlmacen.SelectedItem.Value & "&idprog=" & Me.DropDownList1.SelectedValue.ToString), String)
        End If
