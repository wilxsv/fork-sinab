...
Partial Class Controles_ucVistaListadoSolicitudCompraLG
    ...
    Private Sub obtenerDatos()
        Me.gvSolicitudCompra.DataSource = Me.mComponente.Filtrar_VistaSolitudLG(IDESTABLECIMIENTO, 7, criterio, operador, HttpContext.Current.User.Identity.Name)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            If MessageBox.ConfirmTarget = "Proceso Iniciado" Then InciarProceso()
            If MessageBox.ConfirmTarget = "Rechazar" Then RechazarProceso()
        End If
    ...
    Private Sub obtenerProcesoSugerido(ByVal monto As Decimal)
        ...
        Me.UcListaTipoCompra2._ESTABLECEVALOR = idProcesoCompraSG
        Me.UcListaTipoCompra2.establecerValor()
        Me.UcListaTipoCompra2._ENABLED = False
        Me.UcListaTipoCompra2.habilitarControl()
    End Sub
    ...
    Public Sub validarTipoCompra()
        ...
        Me.ddlProcesoCompraEjecutar.DataSource = ds
        Me.ddlProcesoCompraEjecutar.DataTextField = "DESCRIPCION"
        Me.ddlProcesoCompraEjecutar.DataValueField = "IDTIPOCOMPRA"
        Me.ddlProcesoCompraEjecutar.DataBind()
    ...
    Protected Sub UcBarraNavegacion1_Guardar(ByVal sender As Object, ByVal e As System.EventArgs) Handles UcBarraNavegacion1.Guardar
        MessageBox.Confirm("El proceso de compra ha iniciado satisfactoriamente, desea continuar", "Proceso Iniciado", MessageBox.OptionPostBack.YesPostBack)
        ' Me.MsgBox3.ShowConfirm("El proceso de compra ha iniciado satisfactoriamente, desea continuar", "B", Cooperator.Framework.Web.Controls.ConfirmOption.PostBackOnYes_CallBackOnNo)
    End Sub
    ...
    Protected Sub lbRechazaSolicitud_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbRechazaSolicitud.Click
        MessageBox.Confirm("¿Desea rechazar la(s) solicitud(es) seleccionada(s)?", "Rechazar", MessageBox.OptionPostBack.YesPostBack)
    ...
    Protected Sub btnIniciaProceso_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnIniciaProceso.Click
        ...
        If solSelect.Count > 0 Then
            If validarSolicitudes(solSelect) = True Then
                If aSeleccionado = True Then
                    If j > 1 Then
                        ...
                        MessageBox.Alert("Debe seleccionar solamente una solicitud de compra conjunta ", "Error")
                    ...
                    Else
                        If i > 1 Then
                            If j = 1 Then
                                MessageBox.Alert("Sólo se permite seleccionar una solicitud de compra conjunta", "Error")
                ...
                End If
            Else
                MessageBox.Alert(strNoCumple.ToString, "ERROR")
            ...
            End If
        Else
            MessageBox.Alert("Debe seleccionar una solicitud", "Error")
    ...
    Private Function validarSolicitudes(ByVal ARR_SOLICITUD As listaSOLICITUDES) As Boolean
        ...
        Dim Solic As ABASTECIMIENTOS.ENTIDADES.SOLICITUDES
        ...
        For i = 0 To ARR_SOLICITUD.Count - 1
            ...
            If Solic.IDSOLICITUD <> 0 Then
                ...
                ElseIf idSuministro <> idSuministro_ant Then
                    MessageBox.Alert("No es posible iniciar el proceso para solicitudes de suministros diferentes", "Error")
    ...
    Protected Sub RechazarProceso() 'MsgBox1_YesChosen(ByVal sender As Object, ByVal e As Cooperator.Framework.Web.Controls.MsgBoxEventArgs) Handles MsgBox1.YesChosen
    ...
    Protected Sub InciarProceso()
        Dim lEntidad As New ABASTECIMIENTOS.ENTIDADES.PROCESOCOMPRAS
        ...
        lEntidad.FECHAHORAINICIORETIRO = Format(Me.txtFechaInicioRetiroBases.SelectedDate, "dd/MM/yyyy") & " 07:00 AM " ' & Format(Me.txtHoraInicioRetiroBases.SelectedTime, "HH:mm")
        lEntidad.FECHAHORAFINRETIRO = Format(Me.txtFechaFinRetiroBases.SelectedDate, "dd/MM/yyyy") & " 03:30 PM" '& Format(Me.txtHoraFinRetiroBases.SelectedTime, "HH:mm")

        lEntidad.FECHAHORAINICIORECEPCION = Format(Me.txtFECHAINICIORECEPCION.SelectedDate, "dd/MM/yyyy") & " 07:00 AM " '& Format(Me.txtHORAINICIORECEPCION.SelectedTime, "HH:mm")
        lEntidad.FECHAHORAFINRECEPCION = Format(Me.txtFECHAFINRECEPCION.SelectedDate, "dd/MM/yyyy") & " 03:30 PM" '& Format(Me.txtHORAFINRECEPCION.SelectedTime, "HH:mm")

        lEntidad.FECHAHORAINICIOAPERTURA = Format(Me.txtFECHAINICIOAPERTURA.SelectedDate, "dd/MM/yyyy") & " 07:00 AM " '& Format(Me.txtHORAINICIOAPERTURA.SelectedTime, "HH:mm")
        lEntidad.FECHAHORAFINAPERTURA = Format(Me.txtFECHAFINAPERTURA.SelectedDate, "dd/MM/yyyy") & " 03:30 PM" ' & Format(Me.txtHORAFINAPERTURA.SelectedTime, "HH:mm")

        lEntidad.FECHAPUBLICACION = Format(Me.CalendarPopup1.SelectedDate, "dd/MM/yyyy") & " 07:00 AM" ' & Format(Me.txtHORAFINAPERTURA.SelectedTime, "HH:mm")
    ...
    Private Function lEntidad() As Object
