...
Partial Class Reportes_FrmRptActaAperturaOfertaLG
    ...
    Private Sub ConfiguracionReporte()
        ...
        Dim reportPath As String = Server.MapPath("RptActaAperturaOfertaLG.rpt")
        ...
        For i = 0 To dsProveedor.Tables(0).Rows.Count - 1
            ProveedorRepresentante += " " & dsProveedor.Tables(0).Rows(i).Item("NOMBRE") & "; "
        Next
        ...
        FechaApertura = CDate(Fecha).ToString("d").ToUpper
        ...
        diaAperturaOferta = clsUtilitarios.Num2Text(CDate(FechaApertura).Day).ToUpper
        mesAperturaOferta = Format(CDate(FechaApertura), "MMMM").ToUpper ', clsUtilitarios.Num2Text(CDate(FechaApertura).Month)).ToLower
        añoAperturaOferta = clsUtilitarios.Num2Text(CDate(FechaApertura).Year).ToUpper

        HoraApertura = CDate(Fecha).Hour.ToString.ToUpper

        MinutoApertura = CDate(Fecha).Minute.ToString.ToUpper
        ...
        discreteVal = New ParameterDiscreteValue()
        discreteVal.Value = clsUtilitarios.Num2Text(HoraApertura).ToUpper
        paramField.CurrentValues.Add(discreteVal)
        paramFields.Add(paramField)
